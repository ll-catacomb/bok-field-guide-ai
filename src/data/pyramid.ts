...
export const TIERS: Tier[] = [
  {
    ...
    why: "None of this is about AI, and it still comes first. It is simply good teaching. It is also the audit that makes the next tier possible: once you have said what each assignment is for, you can see where a student could hand the work to a model and short-circuit the learning. And it is the raw material for the tier at the top: clear objectives, readings, rubrics, and worked examples are exactly the context you would load into any AI tool you build for the course. Skip it and the higher tiers have nothing to stand on.",
    practices: [
  ...
  },
  {
    ...
    why: "A take-home essay prompt that a model can answer in ten seconds no longer measures what you think it measures. The Bok Center treats this as a design problem: build work whose value survives the existence of AI, and pair it with an explicit, discussed AI policy. It is far easier once the foundation is solid, because the audit you did there already tells you which assignments are exposed and what each is meant to measure. If a course's goals are vague, you cannot even tell which assignments need protecting.",
    practices: [
